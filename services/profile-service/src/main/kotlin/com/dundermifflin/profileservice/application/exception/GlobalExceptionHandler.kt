package com.dundermifflin.profileservice.application.exception
...
private const val TRACE_ID_ATTR = "profile.traceId"
    ...
    fun handleUnhandled(exception: Exception, request: HttpServletRequest): ResponseEntity<ApiErrorEnvelope> {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
            ApiErrorEnvelope(
                error = ApiError(
                    code = "INTERNAL_ERROR",
                    message = exception.message ?: "Unknown profile-service error",
                    traceId = traceId
                )
            )
        )

...
export function WuphfWidget({ returnTo }: WuphfWidgetProps) {
  ...
  const gatewayBaseUrl = useMemo(
    () => (import.meta.env.VITE_GATEWAY_BASE_URL?.trim() || inferGatewayBaseUrl()).replace(/\/$/, ""),
    []
  );
  ...
  if (loadError) {
    return <p className="portal-wuphf__status">Notifications unavailable: {loadError}</p>;
  }

  if (!isReady) {
    return <p className="portal-wuphf__status">Loading notifications...</p>;
  }
